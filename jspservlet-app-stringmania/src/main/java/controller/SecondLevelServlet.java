...
@WebServlet(urlPatterns = { "/jspservlet-app-stringmania/level2" })
public class SecondLevelServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (seeAnswer != null) {
			if (contains != null) {
				boolean output = stringboolean.containsString(input1, input2);
		...
		if (seeAnswer != null) {
			if (endsWith != null) {
				boolean output = stringboolean.endsWithLetter(input1, input2);
				request.setAttribute("output", output);
				RequestDispatcher dispatcher = request.getRequestDispatcher("level2.jsp");
				dispatcher.forward(request, response);
			}
		...
		if (seeAnswer != null) {
			if (startsWith != null) {
				boolean output = stringboolean.startsWithLetter(input1, input2);
				request.setAttribute("output", output);
				RequestDispatcher dispatcher = request.getRequestDispatcher("level2.jsp");
				dispatcher.forward(request, response);
			}
		...
		if (seeAnswer != null) {
			if (equals != null) {
				boolean output = stringboolean.areEqual(input1, input2);
				request.setAttribute("output", output);
				RequestDispatcher dispatcher = request.getRequestDispatcher("level2.jsp");
				dispatcher.forward(request, response);
			}
		...
		if (seeAnswer != null) {
			if (equalsIgnoreCase != null) {
				boolean output = stringboolean.areEqualIgnoreCase(input1, input2);
				request.setAttribute("output", output);
				RequestDispatcher dispatcher = request.getRequestDispatcher("level2.jsp");
				dispatcher.forward(request, response);
			}
