...
@WebServlet(urlPatterns = { "/jspservlet-app-stringmania/level3" })
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (seeAnswer != null) {
			if (concatenation != null) {
				String output = stringadvance.concatenateStrings(input1, input2);
		...
		if (seeAnswer != null) {
			if (split != null) {
				String[] output = stringadvance.splitStrings(input1, input2);
				request.setAttribute("output", output);
				RequestDispatcher dispatcher = request.getRequestDispatcher("level3.jsp");
				dispatcher.forward(request, response);
			}
		...
		if (seeAnswer != null) {
			if (indexOf != null) {
				int output = stringadvance.findIndexOf(input1, input2);
				request.setAttribute("output", output);
				RequestDispatcher dispatcher = request.getRequestDispatcher("level3.jsp");
				dispatcher.forward(request, response);
			}
